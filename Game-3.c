...
static unsigned char player_x,player_y,screen_x,screen_y,coins_x[5],coins_y[5];//Player x/y coordinates
static unsigned char game_state = 2; // Game State
static unsigned char scroll_speed = 2, sprite_speed = 1;
static unsigned char coins_collected = 0;
struct Laser{
...
void move_coins(){
  ...
  oam_id = oam_spr(coins_x[0],coins_y[0],CH_COIN,0,oam_id);  
  coins_x[1] -= sprite_speed;
  oam_id = oam_spr(coins_x[1],coins_y[1],CH_COIN,0,oam_id);  
  coins_x[2] -= sprite_speed;
  oam_id = oam_spr(coins_x[2],coins_y[2],CH_COIN,0,oam_id);  
  coins_x[3] -= sprite_speed;
  oam_id = oam_spr(coins_x[3],coins_y[3],CH_COIN,0,oam_id);  
}
void coin_collision(){
...
//Concerns movement on screen, gotta make lasers progressively move towards the left
  ...
  for(i = 0; i < lasers[0].num_blocks; i++){
    ...
    if(i == lasers[0].num_blocks - 1)
      ...
      oam_id = oam_spr(lasers[0].blocks_x[i],lasers[0].blocks_y[i],CH_LASER,3,oam_id);
    }
  ...
  for(i = 0; i < lasers[1].num_blocks; i++){
    ...
    if(i == lasers[1].num_blocks - 1)
      ...
      oam_id = oam_spr(lasers[1].blocks_x[i],lasers[1].blocks_y[i],CH_LASER,3,oam_id);
    }
  ...
  for(i = 0; i < lasers[2].num_blocks; i++){
    ...
    if(i == lasers[2].num_blocks - 1)
      ...
      oam_id = oam_spr(lasers[2].blocks_x[i],lasers[2].blocks_y[i],CH_LASER,3,oam_id);
    }
   ...
   for(i = 0; i < lasers[3].num_blocks; i++){
    ...
    if(i == lasers[3].num_blocks - 1)
      ...
      oam_id = oam_spr(lasers[3].blocks_x[i],lasers[3].blocks_y[i],CH_LASER,3,oam_id);
    }
}
